...
% Solve the Minimum Lap Time Problem including the optimal shift in nominal tyre load
...
assert(vp.ActAero ~= 1, ...
    ['MLTP_TyreOptim: ActiveRW (AeroConfig=''ActiveRW'') is only supported by MLTP.m ' ...
     '- its post-processing lacks the data.aeroARW block']);

% Initialisation of the vehicle design parameters and starting velocity with results from the MLTP_paramOptim script
if pt.ATD == 0 && vp.ActAero == 0
importfile('Data\Barcelona\results\AA\Static_ParamOptVi.mat');                          % import initialisation file
data_static = data;
vp.brkB  = data_static.p_opt(1);                                                        % fraction of total brake force going to the front wheels  (-)
vp.Tdist = data_static.p_opt(2);                                                        % fraction of total torque going to the rear wheels        (-)
vp.ksD   = data_static.p_opt(3);                                                        % fraction of total roll stiffness for the rear axle       (-)
vp.alpha_RW = data_static.p_opt(end)*30;
vi = 60.9065;                                                                           % inital velocity static -  tyre case
elseif pt.ATD == 0 && vp.ActAero == 1
importfile('Data\Barcelona\results\AA\ActiveRW_ParamOptVi.mat');                        % import initialisation file
data_activeRW = data;
vp.brkB  = data_activeRW.p_opt(1);                                                      % fraction of total brake force going to the front wheels  (-)
vp.Tdist = data_activeRW.p_opt(2);                                                      % fraction of total torque going to the rear wheels        (-)
vp.ksD   = data_activeRW.p_opt(3);                                                      % fraction of total roll stiffness for the rear axle       (-)
vi = 60.936;                                                                            % inital velocity active RW -  tyre case
elseif pt.ATD == 0 && vp.ActAero == 2
importfile('Data\Barcelona\results\AA\Active_ParamOptVi.mat');                          % import initialisation file
data_active = data;
vp.brkB  = data_active.p_opt(1);                                                        % fraction of total brake force going to the front wheels  (-)
vp.Tdist = data_active.p_opt(2);                                                        % fraction of total torque going to the rear wheels        (-)
vp.ksD   = data_active.p_opt(3);                                                        % fraction of total roll stiffness for the rear axle       (-)
vi = 61.18;                                                                             % inital velocity active -  tyre case
elseif pt.ATD == 0 && vp.ActAero == 3
importfile('Data\Barcelona\results\AA\AALB_ParamOptVi.mat');                            % import initialisation file
data_AALB = data;
vp.brkB  = data_AALB.p_opt(1);                                                          % fraction of total brake force going to the front wheels  (-)
vp.Tdist = data_AALB.p_opt(2);                                                          % fraction of total torque going to the rear wheels        (-)
vp.ksD   = data_AALB.p_opt(3);                                                          % fraction of total roll stiffness for the rear axle       (-)
vi = 61.04;                                                                             % inital velocity AALB - tyre case
elseif pt.ATD == 1 && vp.ActAero == 0
importfile('Data\Barcelona\results\AA_TV\ParamOpt\Static_TV_ParamOptVi.mat');
data_static_TV = data;                                                                  % import initialisation file
vp.ksD   = data_static_TV.p_opt;                                                        % fraction of total roll stiffness for the rear axle       (-)
vp.alpha_RW = data_static_TV.p_opt(end)*30;
vi = 61.695;                                                                            % inital velocity static TV - tyre case
elseif pt.ATD == 1 && vp.ActAero == 1
importfile('Data\Barcelona\results\AA_TV\ParamOpt\ActiveRW_TV_ParamOptVi.mat');         % import initialisation file
data_activeRW_TV = data;
vp.ksD   = data_activeRW_TV.p_opt;                                                      % fraction of total roll stiffness for the rear axle       (-)
vi = 61.695;                                                                            % inital velocity active RW TV - tyre case
elseif pt.ATD == 1 && vp.ActAero == 2
importfile('Data\Barcelona\results\AA_TV\ParamOpt\Active_TV_ParamOptVi.mat');           % import initialisation file
data_active_TV = data;
vp.ksD   = data_active_TV.p_opt;                                                        % fraction of total roll stiffness for the rear axle       (-)
vi = 61.87;                                                                             % inital velocity active TV - tyre case
elseif pt.ATD == 1 && vp.ActAero == 3
importfile('Data\Barcelona\results\AA_TV\ParamOpt\AALB_TV_ParamOptVi.mat');             % import initialisation file
data_AALB_TV = data;
vp.ksD   = data_AALB_TV.p_opt;                                                          % fraction of total roll stiffness for the rear axle       (-)
vi = 61.94;                                                                             % inital velocity AALB TV - tyre case
end
...
if isempty(initFile)
    fprintf('MLTP_TyreOptim: no cached init for %s / %s - running MLTP_initial.m\n', circuit, vp.aeroSetting);
    run('MLTP_initial.m');                  % populates data.init.* AND caches a .mat
else
    fprintf('MLTP_TyreOptim: warm start from %s\n', initFile);
...
% Per-axle nominal tyre-load shift: scales the Pacejka load-sensitivity
% reference (dfz is computed about tyre_f/r.Fz0 * shift in vehModel.m), i.e.
% optimises what nominal load each axle's tyre is rated at.
% NOTE (Zenvo data): with the supplier's gentle load sensitivity (pD2 ~ -0.2),
% grip is monotonically increasing in the shift over the whole operating band,
% so expect bound solutions (shift -> upper bound) rather than an interior
% optimum. The old study's interior optimum was an artefact of the previous
% exaggerated pD2 = -0.6. Retained for comparison with prior work.
Fz0_shift_f_n = SX.sym('Fz0_shift_f_n');
Fz0_shift_r_n = SX.sym('Fz0_shift_r_n');
Fz0_shift_s = 2;
Fz0_shift_lim = 1/Fz0_shift_s * [0.5 1.5];

p_s = [Fz0_shift_s; Fz0_shift_s];
p = [Fz0_shift_f_n; Fz0_shift_r_n];
p_lim = [Fz0_shift_lim; Fz0_shift_lim];
...
if pt.ATD == 0 && vp.ActAero == 0
    ...
    p0 = [1; 1]/Fz0_shift_s;   % both axle shifts start at 1.0 (normalised)
elseif pt.ATD == 0 && vp.ActAero == 1
...
% Note data.p_opt is stored NORMALISED - undo the p_s scaling for the shifts.
Fz0eff_f_opt = vp.tyre_f.Fz0 * data.p_opt(1)*p_s(1);
Fz0eff_r_opt = vp.tyre_r.Fz0 * data.p_opt(2)*p_s(2);
...
data.vehicle.gu_fl = gripUtil(Ftyres,1,vp.tyre_f,Fz0eff_f_opt);
data.vehicle.gu_fr = gripUtil(Ftyres,2,vp.tyre_f,Fz0eff_f_opt);
data.vehicle.gu_rl = gripUtil(Ftyres,3,vp.tyre_r,Fz0eff_r_opt);
data.vehicle.gu_rr = gripUtil(Ftyres,4,vp.tyre_r,Fz0eff_r_opt);
clear gripUtil Fz0eff_f_opt Fz0eff_r_opt
...
for i = 1:length(p)
    ...
    % de-normalise for display (data.p_opt is stored normalised; shift 1.0
    % would otherwise show as 0.5 with p_s = 2)
    data.sdi.solData.parameters.(name) = timeseries(repmat(data.p_opt(i)*p_s(i),length(s_full),1), s_full,'name',name);
end
...
for i = 1:length(p)
    aux = p(i);
    pstr = [pstr erase(aux.name,'_n') '=' num2str(data.p_opt(i)*p_s(i)) '_'];   % de-normalised
end
clear aux
